...
export async function readMessageUpdateCursor(database, conversationID) {
...
export async function writeMessageUpdateCursor(database, conversationID, cursor) {
    if (!/^\d+$/.test(cursor)) {
        throw createWebIMSyncError('INVALID_MESSAGE_UPDATE_CURSOR', 'Message update cursor must be an unsigned decimal string.');
    }
    // cursorKey 与读取路径完全一致。
    const cursorKey = createCursorKey(conversationID);
...
function createCursorKey(conversationID) {
    // normalizedConversationID 禁止空 cursor owner。
    const normalizedConversationID = conversationID.trim();
    if (!normalizedConversationID) {
        throw createWebIMSyncError('INVALID_MESSAGE_UPDATE_CONVERSATION', 'Message update cursor requires a conversation ID.');
    }
    return `${MESSAGE_UPDATE_CURSOR_PREFIX}${normalizedConversationID}`;
}
//# sourceMappingURL=message-update-cursor-store.js.map