...
/** 批量获取用户资料 批量返回经过各目标用户权限设置处理的可见资料；不用于获取本人完整资料。 POST /v1/user/batch-detail */
...
/** 当前用户详情 使用当前登录 token 获取自己的完整用户资料和权限配置；请求体不需要传 user_id。 POST /v1/user/current/detail */
...
/** 用户详情 按用户 ID 查询经过目标用户权限设置处理的可见资料，并返回当前登录用户与目标用户是否为好友关系；查询本人完整资料请使用 `/v1/user/current/detail`。 POST /v1/user/detail */
...
//# sourceMappingURL=yonghu.d.ts.map