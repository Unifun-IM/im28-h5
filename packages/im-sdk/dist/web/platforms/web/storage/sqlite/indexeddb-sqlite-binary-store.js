...
export function createIndexedDBSQLiteBinaryStore(options) {
    // 存储容器名与账号数据库名分离，便于同源内统一管理多账号快照。
    const storageDatabaseName = options.storageDatabaseName ?? 'im28-h5-sqlite-storage';
    return {
        ...
        async write(databaseName, bytes) {
            ...
            try {
                ...
                const transaction = database.transaction(SQLITE_DATABASE_STORE, 'readwrite');
...
//# sourceMappingURL=indexeddb-sqlite-binary-store.js.map