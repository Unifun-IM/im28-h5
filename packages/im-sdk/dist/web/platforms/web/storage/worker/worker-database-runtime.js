...
import { createWorkerDatabaseFailure, createWorkerDatabaseSuccess, parseWorkerDatabaseRequest, } from './worker-database-protocol.js';
...
export function createWorkerDatabaseRuntime(dependencies) {
...
export class WorkerDatabaseRuntime {
    ...
    async handle(value) {
        ...
        try {
            ...
            const isFatal = cause instanceof SqlJsPersistenceError || request.operation === 'open';
    ...
    async dispatch(request) {
        switch (request.operation) {
            ...
            case 'execute':
                return this.requireDatabase().execute(parseStatementPayload(request.payload));
            case 'query':
                return this.requireDatabase().query(parseStatementPayload(request.payload));
    ...
    async beginTransaction(requestID) {
    ...
    requireTransactionStatement(payload) {
        ...
        const statement = parsedPayload.statement;
        return {
    ...
    async finishTransaction(payload, shouldCommit) {
...
function parseStatementPayload(payload) {
    // Zod 已拒绝不可克隆或不受支持的参数类型。
    const parsedPayload = STATEMENT_PAYLOAD_SCHEMA.parse(payload);
    return parsedPayload.statement;
}
//# sourceMappingURL=worker-database-runtime.js.map