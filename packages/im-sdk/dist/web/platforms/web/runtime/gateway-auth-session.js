...
export function normalizeGatewayAuthSession(authData, fallbackUserID = '') {
    ...
    const userID = authData.user?.user_id?.trim() ||
        authData.token?.subject_id?.trim() ||
        fallbackUserID.trim();
    if (!accessToken || !userID) {
        throw new WebIMRuntimeError('INVALID_AUTH_RESPONSE', 'Gateway auth response is missing user or access token.');
    }
...
//# sourceMappingURL=gateway-auth-session.js.map