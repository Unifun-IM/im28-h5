...
/** Worker 数据库 RPC 操作集合。 */
export type WorkerDatabaseOperation = z.infer<
  typeof WORKER_DATABASE_OPERATION_SCHEMA
>;

/** Worker 数据库 RPC 请求 envelope。 */
export type WorkerDatabaseRequest = z.infer<
  typeof WORKER_DATABASE_REQUEST_SCHEMA
>;

/** Worker 数据库 RPC 错误。 */
export type WorkerDatabaseRPCError = z.infer<
  typeof WORKER_DATABASE_ERROR_SCHEMA
>;

/** Worker 数据库 RPC 响应 envelope。 */
export type WorkerDatabaseResponse = z.infer<
  typeof WORKER_DATABASE_RESPONSE_SCHEMA
>;

/** 校验主线程发往 Worker 的请求。 */
export function parseWorkerDatabaseRequest(
  value: unknown,
...
/** 校验 Worker 返回主线程的响应。 */
export function parseWorkerDatabaseResponse(
  value: unknown,
): WorkerDatabaseResponse {
  return WORKER_DATABASE_RESPONSE_SCHEMA.parse(value);
}

/** 创建具名成功响应。 */
export function createWorkerDatabaseSuccess(
  id: string,
  result: unknown,
): WorkerDatabaseResponse {
...
/** 创建具名失败响应并规范化未知异常。 */
export function createWorkerDatabaseFailure(
  id: string,
  cause: unknown,
  fatal: boolean,
): WorkerDatabaseResponse {
