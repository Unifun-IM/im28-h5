...
import {
  parseWorkerDatabaseResponse,
  type WorkerDatabaseOperation,
  type WorkerDatabaseRequest,
  type WorkerDatabaseResponse,
} from './worker-database-protocol.js';
import {
  WorkerDatabaseError,
  type WorkerDatabaseAdapterOptions,
  type WorkerDatabasePort,
} from './worker-database-types.js';

/** 创建在 Dedicated Worker 中执行 SQL 的 DatabaseAdapter。 */
export function createWorkerDatabaseAdapter(
  options: WorkerDatabaseAdapterOptions,
): DatabaseAdapter {
...
class WorkerDatabaseAdapter implements DatabaseAdapter {
  readonly name: string;

  // 创建参数包含 Worker factory，不隐式回退 caller-thread adapter。
  private readonly options: WorkerDatabaseAdapterOptions;
  // 单个 adapter 生命周期只创建一个 Worker。
  private readonly worker: WorkerDatabasePort;
  ...
  private state: 'new' | 'ready' | 'faulted' | 'closed' = 'new';

  /** 创建 Worker 并绑定唯一消息监听器。 */
  constructor(options: WorkerDatabaseAdapterOptions) {
  ...
  transaction<Result>(
    run: (transaction: DatabaseTransaction) => Promise<Result>,
  ): Promise<Result> {
    return this.runSerialized(async () => {
      await this.openDirect();
      // Worker 返回的 transactionID 约束所有 transaction 子操作。
      const transactionID = await this.sendRequest<string>(
        'transaction.begin',
        null,
      );
      ...
      const transaction: DatabaseTransaction = {
        execute: statement =>
          enqueueTransactionOperation(() =>
            this.sendRequest('transaction.execute', {
              transactionID,
              statement,
            }),
          ),
        query: statement =>
          enqueueTransactionOperation(() =>
            this.sendRequest('transaction.query', {
              transactionID,
              statement,
            }),
          ),
  ...
  private runSerialized<Result>(run: () => Promise<Result>): Promise<Result> {
  ...
  private async openDirect(): Promise<void> {
    ...
    if (this.state !== 'new') {
      throw new WorkerDatabaseError(
        ...
        `Worker database cannot open from ${this.state} state.`,
        false,
      );
    }
  ...
  private sendRequest<Result>(
    ...
    return new Promise<Result>((resolve, reject) => {
      ...
      const timeout = setTimeout(
        () =>
          this.faultWorker(
            new WorkerDatabaseError(
              'WORKER_DATABASE_TIMEOUT',
              `Worker database request timed out: ${operation}.`,
              false,
            ),
          ),
  ...
  private readonly handleMessage = (event: MessageEvent): void => {
    ...
    if (!pendingRequest) {
      this.faultWorker(
        new Error(`Unexpected Worker database response: ${response.id}.`),
      );
      return;
    }
    ...
    const cause = new WorkerDatabaseError(
      response.error.code,
  ...
  private faultWorker(cause: unknown): void {
