...
class SqlJsIndexedDBDatabaseAdapter implements DatabaseAdapter {
  readonly name: string;

  // Adapter 配置只持有依赖和账号数据库名，不包含鉴权信息。
  private readonly options: SqlJsIndexedDBDatabaseAdapterOptions;
  ...
  async query<Row extends DatabaseRow = DatabaseRow>(
    ...
    return this.runSerialized(async () => {
      ...
      const database = await this.requireDatabaseDirect();
      return queryStatement<Row>(database, statement);
    });
  }
...
function queryStatement<Row extends DatabaseRow>(
  database: Database,
  statement: DatabaseStatement,
): readonly Row[] {
  ...
  const preparedStatement = database.prepare(
    statement.sql,
    normalizeStatementParameters(statement),
  );
  ...
  try {
    while (preparedStatement.step()) {
      ...
      const row: Record<string, DatabaseValue | undefined> = {};
...
function normalizeStatementParameters(
  statement: DatabaseStatement,
): SqlValue[] {
...
function normalizeQueryValue(value: SqlValue): DatabaseValue {
