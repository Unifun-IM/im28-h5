...
import { IMError, type GatewayFetch } from '@im28/im-sdk/web';
import { z } from 'zod';
...
/** 页面只允许查询已冻结的平台条款业务键。 */
export type WebIMPlatformTermKey =
  (typeof WEB_IM_PLATFORM_TERM_KEYS)[keyof typeof WEB_IM_PLATFORM_TERM_KEYS];
...
export function createWebIMPlatformTermsClient(
  options: WebIMPlatformTermsClientOptions,
): WebIMPlatformTermsClient {
  return {
    /** 查询并验证单个已启用的平台条款。 */
    async getTerm(key: WebIMPlatformTermKey): Promise<WebIMPlatformTerm> {
      ...
      const payload = await gatewayOpenAPI.apIpingtai.postV1PlatformTermGet(
        { key },
        {
      ...
      if (!parsedEnvelope.success) {
        throw new WebIMRuntimeError(
          ...
          'Gateway platform term response is invalid.',
