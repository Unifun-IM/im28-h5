...
import {
  ...
  type WebIMRuntimeEvent,
  type WebIMRuntimeState,
...
export function createWebIMRuntime(
  options: WebIMRuntimeOptions,
): WebIMRuntime {
...
class WebIMRuntimeImpl implements WebIMRuntime {
  ...
  /** 创建动态绑定当前 auth/account DB owner 的聚合同步入口。 */
  getSync() {
    return createWebIMSync({
