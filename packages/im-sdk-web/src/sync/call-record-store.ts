...
export async function ensureCallSchema(database: DatabaseAdapter): Promise<void> {
...
export async function replaceCachedCalls(
  database: DatabaseAdapter,
  calls: readonly GatewayCall[],
): Promise<void> {
  await database.transaction(async transaction => {
    await transaction.execute({ sql: 'DELETE FROM call_records' });
    // updatedAt 为本次完整 snapshot 使用同一写入时间。
    const updatedAt = Date.now();
    for (const call of calls) await upsertCachedCall(transaction, call, updatedAt);
  });
}

/** 删除服务端已确认隐藏的本地通话记录。 */
export async function removeCachedCalls(
  database: DatabaseAdapter,
...
/** 查询缓存列表并复用同一筛选条件统计总数。 */
export async function queryCachedCalls(
  database: DatabaseAdapter,
  ...
  const totalRows = await database.query<{ readonly total?: DatabaseValue }>({
...
async function upsertCachedCall(
...
/** 构造状态与昵称/用户 ID 搜索条件。 */
function buildCallQuery(options: WebIMCallListOptions): CallQuery {
...
function mapCallRow(row: Readonly<Record<string, DatabaseValue | undefined>>): GatewayCall {
...
/** 归一化服务端接听分类值。 */
function normalizeAnswerStatus(value: unknown): 'answered' | 'missed' | '' {
...
/** 将 SQLite 值收敛为字符串。 */
function readString(value: DatabaseValue | undefined): string {
