import {
  ...
  type GatewayMessageUpdate,
  type GatewayRealtimeEvent,
  type Message,
} from '@im28/im-sdk/web';

import {
  readMessageUpdateCursor,
  writeMessageUpdateCursor,
} from './message-update-cursor-store.js';
...
export function createRealtimeMessageUpdateSync(
  dependencies: RealtimeMessageUpdateSyncDependencies,
) {
  ...
  return async (
    ...
    if (!eventUpdates.length) {
      throw createWebIMSyncError(
        'INVALID_REALTIME_MESSAGE_UPDATE',
        'Realtime message update has no stable operation identity.',
      );
    }
    ...
    for (const [conversationID, updates] of groups) {
      await convergeConversationUpdates(
...
async function convergeConversationUpdates(
  ...
  let storedCursor = await readMessageUpdateCursor(
    context.database,
    conversationID,
  );
  // recovery 在需要时补齐服务端窗口，并返回去重排序结果。
  const recovery = await recoverRealtimeMessageUpdates(
    gatewayClient,
    conversationID,
    storedCursor,
    eventUpdates,
  ...
  for (const parsed of recovery.updates) {
    ...
    if (parsed.updateSeq) {
      await writeMessageUpdateCursor(
        context.database,
        conversationID,
        parsed.updateSeq,
      );
...
async function applyMessageUpdate(
  ...
  context: WebIMSyncContext,
  parsed: ParsedRealtimeMessageUpdate,
): Promise<void> {
  ...
  if (update.type !== 'edited' || !update.message) {
    throw createWebIMSyncError(
      'INVALID_EDITED_MESSAGE_UPDATE',
      'Edited message update requires a complete target message.',
    );
  }
  ...
  if (existing && !shouldApplyEditedMessage(existing, parsed)) return;
  // editedMessage 保留本地 identity/order/status，只替换服务端内容。
  const editedMessage: Message = existing
    ? {
        ...mapped,
...
async function applyDeletedUpdate(
  repository: MessageRepository,
  update: GatewayMessageUpdate,
): Promise<void> {
  // target 优先按 Gateway target server ID 查找。
  const target = await findUpdateTarget(repository, update);
  if (target) await repository.markLocalDeleted(target.clientMsgID);
}
...
async function findUpdateTarget(
  ...
  const clientMsgID =
    readString(update.message?.client_msg_id) ?? mapped?.clientMsgID;
  return clientMsgID
...
function createEditedLocalExtra(
  ...
  try {
    ...
    const candidate = current ? (JSON.parse(current) as unknown) : null;
    if (candidate && typeof candidate === 'object' && !Array.isArray(candidate)) {
      existing = candidate as Record<string, unknown>;
    }
  } catch {
    existing = {};
  }
...
function shouldApplyEditedMessage(
  existing: Message,
  parsed: ParsedRealtimeMessageUpdate,
): boolean {
  // incomingEditedAt 缺失时无法比较，只保留幂等 upsert 行为。
  const incomingEditedAt = readEditedAt(parsed);
  if (!incomingEditedAt || !existing.localEx) return true;
  try {
    // metadata 只读取本 owner 写入的 editedAt 数值。
    const metadata = JSON.parse(existing.localEx) as Record<string, unknown>;
...
function readEditedAt(parsed: ParsedRealtimeMessageUpdate): number {
...
function groupUpdatesByConversation(
  updates: readonly ParsedRealtimeMessageUpdate[],
): Map<string, ParsedRealtimeMessageUpdate[]> {
  // groups 保留会话首见顺序。
  const groups = new Map<string, ParsedRealtimeMessageUpdate[]>();
