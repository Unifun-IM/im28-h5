...
import {
  collectGatewayConversations,
  collectGatewayMessages,
  deduplicateGatewayMessages,
  groupGatewayMessages,
  hasDegradedMarker,
  hasSequenceGap,
  maxDecimalString,
  persistMappedMessages,
  readString,
  selectLatestMessage,
} from './realtime-event-data.js';
...
class WebIMRealtimeSyncImpl implements WebIMRealtimeSync {
  ...
  private readonly handleMessageUpdate;
  ...
  handle(event: GatewayRealtimeEvent): Promise<boolean> {
    ...
    return this.mutationQueue.enqueue(() =>
      this.handleDirect(event, context),
    );
  }
  ...
  private async persistConversationMessages(
    ...
    const recoveredMessages = recoveryNeeded
      ? await pullRealtimeMessageRecovery(
          this.dependencies.gatewayClient,
          conversationID,
  ...
  private async updateExistingConversation(
    ...
    gatewayMessages: readonly GatewayMessage[],
    mappedMessages: Parameters<typeof selectLatestMessage>[0],
    unreadDelta: number,
  ): Promise<void> {
