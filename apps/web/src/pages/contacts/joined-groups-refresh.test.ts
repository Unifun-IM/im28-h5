...
describe('joined groups pull refresh contract', () => {
  ...
  it('reuses pull refresh and the shared group sync owner', () => {
    ...
    expect(joinedGroupsSource).toContain("runtime.getSync().groups.sync({ pageSize: 50 })");
    expect(joinedGroupsSource).not.toMatch(/GatewayHTTPClient|GroupRepository|@openim\//);
  });

  /** 刷新状态独立于搜索词，列表仍由本地纯投影负责过滤。 */
  it('preserves local search and visible refresh feedback', () => {
    expect(joinedGroupsSource).toContain('filterJoinedGroups(groups, keyword)');
