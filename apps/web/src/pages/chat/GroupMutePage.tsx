...
export function GroupMutePage() {
  ...
  return (
    ...
    >
      ...
      <InteractionModal open={Boolean(pickerTarget)} ariaLabel="选择禁言时长" placement="bottom" onRequestClose={() => setPickerTarget(null)}><section className="rn-group-action-picker im-modal-sheet"><h2>选择禁言时长</h2>{MEMBER_MUTE_OPTIONS.map(option => <button type="button" key={option.seconds} onClick={() => { if (pickerTarget) setAction({ type: 'member', userID: pickerTarget, seconds: option.seconds }); setPickerTarget(null); }}>{option.label}</button>)}<button type="button" onClick={() => setPickerTarget(null)}>取消</button></section></InteractionModal>
