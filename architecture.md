...
- 群成员完整页与群设置成员预览只用 shared `WebIMJoinedGroup.mode` 和 `isIMNormalGroupMode` 决定 presence 可见性：仅普通群通过同一个 `useGroupMemberPresence -> presence.observe` 观察各自实际渲染成员，large/unknown 群 fail-closed。在线映射只存在页面 hook 内存，不写成员 DTO/SQLite，不增加 WebSocket listener；成员昵称、角色、资料路由和下拉同步继续走既有 owner。
...
- `/conversations`、`/contacts`、`/calls` 与 `/me` 共享 `PrimaryTabsLayout -> PrimaryTabBar` 唯一底栏；chat detail、`/me/settings`、auth 与 404 不挂载底栏，禁止复制页面级 tabbar。主布局唯一持有好友申请未读与群申请总数快照，并通过 Provider 同时供通讯录 Tab 角标和 ContactsPage shortcut 消费；同账号并发刷新合并，旧账号结果不得回写。已选消息 Tab 在 320ms 内二次点击且存在非静音未读时，只调用当前会话页注册的只读滚动端口；会话页按 RN 规则把 `unreadCount > 0` 或 `manualUnread` 视为候选，从首个可见行之后选择并记住上次目标循环。该交互不调用 Gateway、Repository 或已读 mutation。
